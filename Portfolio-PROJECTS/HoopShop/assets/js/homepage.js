...
const products = [
  ...
  // BALL 
  {
      ...
      brand: "Wilson",
      name: "WILSON NBA DRV Series Outdoor Basketball",
  ...
  },
  {
      ...
      brand: "Jordan",
      signature: "Jordan",
      name: "Jordan Playground 8P Basketball",
  ...
  },
  {
      ...
      brand: "Nike",
      signature: "Nike",
      name: "Nike Premium Energy Basketball",
  ...
  },
  {
      ...
      brand: "Nike",
      signature: "Giannis",
      name: "Giannis Playground 8P Basketball",
  ...
  },
  {
      ...
      brand: "Nike",
      signature: "Nike",
      name: "Nike Elite Championship 8P Basketball",
...
document.addEventListener('DOMContentLoaded', function() {
    let currentSlide = 0;
    const slides = document.querySelectorAll('.home-main-banner .slide');
    const totalSlides = slides.length;
    let slideInterval = setInterval(nextSlide, 6000); // Initialize the automatic slideshow
    const pauseDuration = 10000; // Pause for 10 seconds after manual change

    const indicators = document.querySelectorAll('.home-main-banner .indicator');
    ...
    function updateSlide(newSlide) {
        ...
        setActiveIndicator(newSlide); // Set the active indicator
        ...
        // Restart the interval for automatic slideshow
    ...
    document.querySelector('.next-slide').addEventListener('click', function() {
        ...
        clearInterval(slideInterval);
        slideInterval = setTimeout(startSlideShow, pauseDuration);
    });
    ...
    // Set up initial active indicator
...
function displayProducts(products) {
    const container = document.querySelector('.new-release-section');
    container.innerHTML = ''; // Clear existing content

    products.forEach(product => {
      const mainImageUrl = product.imageUrls[0];
      let sizeOptions = product.sizes ? product.sizes.map(size => `<option value="${size}">${size}</option>`).join('') : '';

      const itemHtml = `
        <div class="item">
          <a href="./pages/product-page.html?productId=${product.id}">
    ...
    // After the products are displayed, set up the scrolling
    setupScrolling();
}
...
function setupScrolling() {
  ...
  const calculateScrollValues = () => {
    const itemStyle = window.getComputedStyle(document.querySelector('.item'));
    ...
    scrollPerClick = itemWidth + itemMargin;

    const itemsLength = document.querySelectorAll('.item').length;
    const newReleaseWrapperWidth = document.querySelector('.new-release-wrapper').offsetWidth;
    const visibleItems = Math.floor(newReleaseWrapperWidth / scrollPerClick);
    maxScroll = -scrollPerClick * (itemsLength - visibleItems);
    // Prevent excessive scrolling on small screens
  ...
  slideRightButton.addEventListener('click', () => {
    if (scrollAmount > maxScroll) {
      ...
      newReleaseSection.style.transform = `translateX(${scrollAmount}px)`;
    }
  });
...
// Define the IDs you want to display in the desired order
const displayIds = [10, 16, 41, 42, 15, 11, 2, 3, 38, 39];

// Filter the products array
const filteredProducts = products.filter(product => displayIds.includes(product.id));

// Sort the filtered products based on the order in displayIds
const sortedProducts = filteredProducts.sort((a, b) => {
  return displayIds.indexOf(a.id) - displayIds.indexOf(b.id);
});

// Call the displayProducts function with the sorted products
...
results.forEach(result => {
  ...
  linkElement.href = `./pages/product-page.html?productId=${result.id}`; // Point the link to the product detail page with the product ID
...
document.querySelector('.header-search-bar form').addEventListener('submit', function(event) {
  ...
  window.location.href = `./pages/all-products-page.html?query=${encodeURIComponent(searchText)}`; // Redirect to the all products page with search query
});

// Event listeners for navigation buttons
document.getElementById('nav-shoes').addEventListener('click', function(event) {
  event.preventDefault();
  filterProductsByType('Shoe');
});

document.getElementById('nav-clothing').addEventListener('click', function(event) {
  event.preventDefault();
  filterProductsByType('OffCourt'); // Assuming Clothing and Off-Court are the same
});

document.getElementById('nav-offcourt').addEventListener('click', function(event) {
  event.preventDefault();
  filterProductsByType('OffCourt');
});

document.getElementById('nav-equipment').addEventListener('click', function(event) {
  event.preventDefault();
  filterProductsByType('Ball');
});

document.getElementById('nav-all').addEventListener('click', function(event) {
  event.preventDefault();
  window.location.href = './pages/all-products-page.html';
});

function filterProductsByType(type) {
  window.location.href = `./pages/all-products-page.html?type=${type}`;
}
...
// MAP
var map = L.map('mapid').setView([49.8951, -97.1384], 13); // Coordinates for Winnipeg
...
// Adding a marker for the first store location
var storeLocation1 = L.marker([49.80643, -97.15506]).addTo(map); // Replace with your first store's coordinates
storeLocation1.bindPopup("<b>Pembina Hwy</b><br>Call 1-(234)-567-8999.").openPopup();

// Adding a marker for the second store location (St. Vital Centre in this example)
var storeLocation2 = L.marker([49.82774, -97.11092]).addTo(map); // Replace with your second store's coordinates
storeLocation2.bindPopup("<b>St. Vital Centre</b><br>Call 1-(234)-567-8999.").openPopup();

// Function to reset map view
function resetMapView() {
  var bounds = new L.LatLngBounds([storeLocation1.getLatLng(), storeLocation2.getLatLng()]);
...
window.onload = function() {
  resetMapView(); // Call this function to set the map view when the page loads
};
  ...
  if (cartItems.length === 0) {
    ...
    cartItems.forEach(item => {
      ...
      miniCartItem.innerHTML = `
        <div class='cart-item'>
        <a href="./pages/product-page.html?productId=${item.productId}" class="mini-cart-link">
