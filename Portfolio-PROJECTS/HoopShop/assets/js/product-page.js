...
let selectedProduct; // Global Variable for cart

function fetchProductDetails(productId) {
    console.log("Fetching details for product ID:", productId);
    // Find the product in the array using the productId
...
function updateProductPage(product) {
  ...
  selectedProduct = product;
    ...
    document.querySelector('.main-image').src = product.imageUrls[0];
    document.querySelector('.product-description').textContent = product.description;
    document.querySelector('.product-price').textContent = product.price;

    // Get the containers for the dropdown and the size buttons
    const sizeDropdownContainer = document.querySelector('#size-dropdown-container');
    const sizeContainer = document.querySelector('#size-container');
    const sizeBoxContainer = document.querySelector('#size-box-container'); // This line was missing


     // Check if the product is a shoe
     if (product.prodType === "Shoe") {
        // Show size boxes for shoes and hide the dropdown
        sizeContainer.style.display = 'block';
        sizeDropdownContainer.style.display = 'none';

        // Clear previous sizes and create new size boxes
        ...
        product.shoeSizes.forEach(size => {
            ...
            sizeBox.textContent = size.US;
            sizeBox.setAttribute('data-value', size.US);
            sizeBoxContainer.appendChild(sizeBox);
        });

        // Attach click handlers to size boxes after they have been created
        attachSizeBoxClickHandlers();
    } else {
        // Hide size boxes for non-shoes and show the dropdown
        sizeContainer.style.display = 'none';
        sizeDropdownContainer.style.display = 'block';

        // Populate the dropdown with sizes for non-shoe items
    ...
    thumbnailContainer.innerHTML = product.imageUrls.map((url, index) =>
        `<img src="${url}" alt="${product.name} thumbnail ${index + 1}" data-index="${index}">`
    ).join('');

    // Add click handler for thumbnails
    attachThumbnailClickHandler(product);
}



// Function to handle thumbnail click
function attachThumbnailClickHandler(product) {
    const thumbnails = document.querySelectorAll('.thumbnail-images img');
    thumbnails.forEach(thumb => {
        thumb.addEventListener('click', function() {
            const index = this.dataset.index;
            const mainImage = document.querySelector('.main-image');
            // Now we use the passed product's imageUrls
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (productId) {
        ...
        // Handle the error if productId is missing or invalid
    ...
    // Since sizeBoxes are dynamically generated, we need to ensure the event listeners are attached after they are created.
    // Therefore, this should be called after fetchProductDetails in which updateProductPage function creates size boxes.
});

// This function will be called after the size boxes are created in the updateProductPage function.
function attachSizeBoxClickHandlers() {
    // Get all size boxes
...
// Function to generate the "You May Also Like" section
function generateYouMayAlsoLike() {
    const numberOfItems = 4; // The number of items you want to display
    const youMayAlsoLikeContainer = document.getElementById('you-may-also-like');
    
    // Randomly pick 4 unique products
    ...
    // Create HTML for each product and append to the container
    selectedProducts.forEach(product => {
      ...
      productThumbnail.onclick = function() {
        window.location.href = './product-page.html?productId=' + product.id;
      };
      youMayAlsoLikeContainer.appendChild(productThumbnail);
    });
}
...
// Handle size selection for shoes and other products
function handleSizeSelection(isShoe, sizes) {
  // Code to handle size selection...
}

// Handle the "Add to Bag" action
function handleAddToBag(productId, quantity, size) {
  const product = products.find(p => p.id === productId);
  if (!product) {
    console.error("Product not found");
    return;
  ...
  // Here you can check if the user is logged in and either update the cart in local storage
  // or send the data to the server.
  // For simplicity, we're just logging the action here.
  console.log(`Adding product ${product.name}, Quantity: ${quantity}, Size: ${size}`);
}
...
async function addToCart(product, quantity, size) {
  ...
  // When adding a new item to the cart, ensure the productId is stored as an integer
  const cartItem = {
    productId: parseInt(product.id, 10), // Store productId as a number
    quantity: quantity,
    size: size,
  };

  // Check if the product with the same size already exists in the cart
  const existingItemIndex = cart.findIndex(item => item.productId === cartItem.productId && item.size === size);
  ...
  // Log the addition of the item to the cart and the current cart contents
  console.log(`Item successfully added to cart: ${product.name}, Quantity: ${quantity}, Size: ${size || 'N/A'}`);
  console.log('Current cart:', cart);

  await updateCartCounter();

  // Check if mini-cart is currently open, and if so, re-render it
...
async function addToCartBackend(product, quantity, size) {
  try {
    ...
    console.log(`Item successfully added to server cart: ${product.name}, Quantity: ${quantity}, Size: ${size}`);
...
document.querySelector('.add-to-cart-btn').addEventListener('click', async function(event) {
  ...
  const quantity = parseInt(quantityInput.value, 10) || 1; // Default to 1 if for some reason the input is not a number

  let size = null; // Default to null for non-shoe products
  if (selectedProduct.prodType === "Shoe") {
    // Logic to get the selected shoe size
    const selectedSizeBox = document.querySelector('.size-box.selected');
    if (selectedSizeBox) {
      size = selectedSizeBox.getAttribute('data-value');
    } else {
      alert("Please select a size.");
      return; // Don't add to cart if no size is selected for shoes
    }
  } else if (selectedProduct.prodType !== "Shoe" && selectedProduct.sizes) {
    // For other sized products
    size = document.getElementById('size-select').value;
  }

  const loggedIn = await isUserLoggedIn();
  if (loggedIn) {
    console.log('User is logged in, handling cart on the server...');
    await addToCartBackend(selectedProduct, quantity, size);
  } else {
    console.log('User is not logged in, handling cart in local storage...');
    addToCart(selectedProduct, quantity, size);
    showAddedToCartPopup(); // Show the popup for non-logged in users
    updateCartCounter(); // Update the counter for non-logged in users
  }
});
