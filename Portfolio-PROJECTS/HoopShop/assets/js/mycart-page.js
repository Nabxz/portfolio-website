...
function createCartItemElement(item) {
  ...
  cartItemElement.innerHTML = `
    <div class="cart-product-details">
      <img src="${product.imageUrls[0]}" alt="${product.name}" class="cart-item-image">
      <div>
        <h4 class="cart-item-name">${product.name}</h4>
        <p>Size: ${item.size || 'N/A'}</p>
        <p>Quantity: ${item.quantity}</p>
        <p>Total: CA$${totalPrice}</p>
        <button onclick="removeFromCart(${productId}, '${item.size}')" class="cart-item-remove">Remove</button>
      </div>
    </div>
  `;
...
async function renderCart() {
  ...
  if (cartItems.length === 0) {
    cartContainer.textContent = 'No products in the cart.';
    cartContainer.style = 'text-align: center; padding-top: 50px; padding-bottom: 50px; color: gray';
  } else {
    let subtotal = 0;
    cartItems.forEach(item => {
      ...
      if (cartItemElement) {
        ...
        const productPrice = parseFloat(product.price.replace('CA$', ''));
...
// Function to update the cart summary (subtotal, taxes, and total)
function updateCartSummary(subtotal) {
  // Assuming we have a function to calculate taxes and total
  const { gst, pst, total } = calculateTaxesAndTotal(subtotal);

  document.getElementById('subtotal-value').textContent = subtotal.toFixed(2);
  document.getElementById('gst-cart').textContent = gst.toFixed(2);
  document.getElementById('pst-cart').textContent = pst.toFixed(2);
