    ...
    const rowsHtml = data.planets
      .map(item => {
        ...
        return `<tr>
                ...
                <td>${dateAdded}</td>
                <td>
                    <a href="./planet-viewer-page.html?planet=${encodeURIComponent(
                      item.planet_name
                    )}" class="details-button">Planet Details</a>
                    <button onclick="showEditNoteModal('${
                      item.planet_name
                    }', '${attributeNote}')" class="details-button">Edit Note</button>
    ...
    fetchUserPlanets()
      .then(data => {
        if (data.success) {
          ...
          displayUserPlanets(
            { success: true, planets: sortedPlanets },
            selectedOption
          )
        }
      })
...
function queryAndDisplayResearchPlanets (query) {
  ...
  const searchApiUrl = `https://galaxy.mammani.com/api/planetsv2?name=${encodeURIComponent(
    query
  )}`
...
function displayResearchResults (results) {
  ...
  if (!resultsContainer) {
    ...
    document
      .querySelector('.research-input-container')
  ...
  if (results && results.length > 0) {
    results.forEach(planet => {
      ...
      planetElement
        ...
        .addEventListener('click', () => addPlanetPrompt(planet.name))
...
function addPlanet (planetName, withNote) {
  const data = {
    planetName,
    note: withNote ? document.getElementById('note-text').value : ''
  }
...
function addPlanetPrompt (planetName) {
  ...
  document
    .querySelector('#note-modal .modal-close-button')
    .addEventListener('click', () => {
      noteModal.style.display = 'none'
      noteModal.remove()
    })
  ...
  fetch('http://127.0.0.1:3000/user-details', {
    ...
    .then(data => {
      if (data.success) {
        ...
        const accountDisplayElement =
          document.getElementById('my-account-display')
        ...
        function displayEmailChangeForm () {
          ...
          document.getElementById('change-account-details-div').innerHTML =
            emailFormHtml
          document
            .getElementById('email-change-form')
            .addEventListener('submit', handleEmailChange)
        }
        ...
        function displayNameChangeForm () {
          ...
          document.getElementById('change-account-details-div').innerHTML =
            nameFormHtml
          document
            .getElementById('name-change-form')
            .addEventListener('submit', handleNameChange)
        }
        ...
        function updateUserDetails (apiEndpoint, updateData) {
          fetch('https://galaxy.mammani.com' + apiEndpoint, {
            ...
            .then(data => {
              if (data.success) {
                ...
                if (data.message.includes('Old email does not match')) {
                  showPopupMessage(
                    'The current email you entered does not match our records. Please try again.',
                    true
                  )
                } else if (
                  data.message.includes('Old password does not match')
                ) {
                  showPopupMessage(
            ...
            .catch(error => {
              ...
              showPopupMessage(
                'An unexpected error occurred while updating your details. Please try again later.',
                true
              )
            })
        }
...
document
  .querySelector('.sidebar-link[href="#research"]')
  ...
  sortSelectorHtml = `
    ...
    <select id="sort-selector" aria-label="Sort reading list">
        <option value="name-asc"${
          sortOption === 'name-asc' ? ' selected' : ''
        }>Title: Ascending</option>
        <option value="name-desc"${
          sortOption === 'name-desc' ? ' selected' : ''
        }>Title: Descending</option>
        <option value="date-asc"${
          sortOption === 'date-asc' ? ' selected' : ''
        }>Date Published: Oldest First</option>
        <option value="date-desc"${
          sortOption === 'date-desc' ? ' selected' : ''
        }>Date Published: Newest First</option>
    </select>`
    ...
    const rowsHtml = sortedList
      .map(item => {
        ...
        const article = articlesArray.find(
          article => article.id === item.product_id
        )
        if (article) {
          return `<tr>
                <td>
...
function fetchAndDisplayReadingList (sortOption) {
  fetchReadingList()
    .then(data => {
      ...
      let enrichedAndSortedList = enrichAndSortReadingList(
        data.wishlist,
        sortOption
      )
      displayReadingList(
        { ...data, wishlist: enrichedAndSortedList },
        sortOption
      )
    })
    .catch(error => {
  ...
  fetch('http://127.0.0.1:3000/wishlist/remove', {
    ...
    .then(data => {
      if (data.success) {
        ...
        showPopupMessage(
          `"${articleTitle}" has been removed from your reading list.`,
          false
        )
        ...
        showPopupMessage(
          `Failed to remove "${articleTitle}" from your reading list.`,
          true
        )
      }
    })
    .catch(error => {
      ...
      showPopupMessage(
        `Error removing "${articleTitle}" from your reading list.`,
        true
      )
    })
}
...
document
  .querySelector('.planet-search-input')
  .addEventListener('input', function (e) {
    ...
    if (userInput.length > 0) {
      ...
      document
        .querySelector('.planet-search-container')
        .classList.add('planet-search-active')
      searchPlanets(userInput) // Function to search planets
    } else {
      // Remove the 'search-active' class when input is empty
      document
        .querySelector('.planet-search-container')
        .classList.remove('planet-search-active')
    }
  })
...
function searchPlanets (query) {
  ...
  fetch(apiURL, {
    ...
    .then(data => {
      displaySearchResults(data) // Pass the results to your UI
    })
    .catch(error => {
...
function displaySearchResults (results) {
  ...
  if (oldResults) {
    document
      .querySelector('.planet-search-container')
      .replaceChild(resultsContainer, oldResults)
  } else {
    document
      .querySelector('.planet-search-container')
      .appendChild(resultsContainer)
...
document
  ...
  .addEventListener('submit', function (event) {
    ...
    if (email) {
      // Simple validation here, you can add more
