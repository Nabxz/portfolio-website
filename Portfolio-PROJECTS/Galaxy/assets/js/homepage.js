...
function populateHomeLayout () {
  ...
  let spaceWeatherSummary = `
    ...
    <div class="weather-data">
      ${displaySpaceWeatherEvent(
        'Fastest<br>CME',
        '500 km/s',
        'The fastest coronal mass ejection recorded.',
        './assets/images/homepage/weather-icons-cme.webp'
      )}
      ${displaySpaceWeatherEvent(
        'Strongest<br>GMS',
        'Kp 7',
        'A strong geomagnetic storm affecting satellites.',
        './assets/images/homepage/weather-icons-gs.webp'
      )}
      ${displaySpaceWeatherEvent(
        'Largest<br>Solar Flare',
        'X-Class',
        'The most intense solar flare observed.',
        './assets/images/homepage/weather-icons-solar-flare.webp'
      )}
      ${displaySpaceWeatherEvent(
        'SEPs<br>Intensity',
        'High',
        'Significant solar energetic particle activity.',
        './assets/images/homepage/weather-icons-seps.webp'
      )}
    </div>
  `
  ...
  fetch(cmeUrl)
    ...
    .then(cmeData => {
     ...
     <div class="scrollable-list">
       ${cmeData
         ...
         <div class="cme-event">
           ...
           <p><strong>Speed:</strong> ${
             event.speed ? event.speed + ' km/s' : 'N/A'
           }</p>
       ...
       `
        ...
        document.getElementById('space-weather').innerHTML =
          '<p>No recent Coronal Mass Ejections detected.</p>'
      }
    ...
    .then(flrData => {
      ...
      <div class="scrollable-list">
        ...
        flrContent +=
          '<p>No significant solar flares recorded in the past 30 days.</p>'
      }
      ...
      content += `
          <!-- Space Missions Section -->
              ...
              <table class="events-table missions-table">
                      ...
                      <!-- Example mission row -->
                      <tr>
              <td>Multi-Angle Imager for Aerosols (MAIA)</td>
              <td>2026-08-01</td>
              <td>NASA Earth science mission to study air quality.</td>
            </tr>
            <tr>
              <td>Investigation of Convective Updrafts (INCUS)</td>
              <td>2026-10-01</td>
              <td>NASA mission to study storms and precipitation.</td>
            </tr>
            <tr>
              <td>Nancy Grace Roman Space Telescope</td>
              <td>2026-10-30</td>
              <td>NASA flagship mission to study dark energy and exoplanets.</td>
            </tr>
            <tr>
              <td>Intuitive Machines 3 (IM-3)</td>
              <td>2026-12-15</td>
              <td>NASA CLPS lunar lander with PRISM payloads.</td>
            </tr>
          </table>
    </div>
  `
    ...
    .catch(error => {
      ...
      document.getElementById('space-weather').innerHTML +=
        '<p>Error fetching Solar Flare data. Please try again later.</p>'
    })
}
...
function fetchNasaImageOfTheDay () {
  fetch('https://api.nasa.gov/planetary/apod?api_key=' + NASA_API_KEY)
    ...
    .then(data => {
      const imageOfTheDayContainer = document.querySelector(
    ...
    .catch(error => {
      ...
      if (imageOfTheDayContainer) {
        imageOfTheDayContainer.innerHTML =
          '<p>Error loading the NASA Image of the Day.</p>'
      }
    })
}
...
function populateLiveLayout () {
  let sideBarContent = ''
  sideBarContent += `
    <button onclick="changeVideo('https://www.youtube.com/embed/H999s0P1Er0', 'NASA TV Live')">NASA Live</button>
    <button onclick="changeVideo('https://www.youtube.com/embed/xRPjKQtRXR8', 'Live HD View From ISS')">Live HD View From ISS</button>
    ...
    <button onclick="changeVideo('https://www.youtube.com/embed/Jm8wRjD3xVA', 'Live NASA Media Channel')">SpaceX Falcon 9 Operations</button>
  `

  document.getElementById('live-sidebar').innerHTML = sideBarContent

  // Initial content for the main area
  changeVideo('https://www.youtube.com/embed/H999s0P1Er0', 'NASA Live')
}
  ...
  if (event.key === 'Enter') {
    ...
    Promise.all([
      ...
      const combinedResults = results
        .map(result => result.cities || result.countries)
        .flat()
...
async function updateEarthSectionDisplay (data) {
  ...
  fetch(`https://api.api-ninjas.com/v1/weather?lat=${lat}&lon=${lon}`, {
    ...
    .then(([weatherData, timeData]) => {
      ...
      const daylightProgress = updateDaylightProgressBar(
        sunriseTime,
        sunsetTime,
        timeData.datetime
      )
      ...
      <div class="weather-info">
        ...
        <div class="weather-info-section">
          <p> Highs Of ${weatherData.max_temp}°C and Lows Of ${
