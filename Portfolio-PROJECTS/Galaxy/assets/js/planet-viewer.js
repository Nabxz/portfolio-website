...
function fetchPlanetDetails (planetName) {
  ...
  fetch(apiURL, {
    ...
    .then(data => {
      if (data.length > 0) {
        displayPlanetDetails(data[0]) // Display the first result
      } else {
        console.error('No details found for the specified planet.')
        showPopupMessage('No planet details found.', true)
      }
    })
    .catch(error => {
      console.error('Error fetching planet details:', error)
      showPopupMessage('Error fetching planet details.', true)
    })
}
  ...
  const detailsHtml = `
        ...
        <div class="planet-details-box">
            ...
            <div class="box-data">
                ...
                <p><strong>Semi-major Axis:</strong><br> ${
                  planet.semi_major_axis || 'Unknown'
                } AU</p>
                <p><strong>Surface Temperature:</strong><br> ${
                  planet.temperature || 'Unknown'
                } K</p>
            </div>
        </div>
        ...
        <div class="planet-details-box">
            ...
            <div class="box-data">
                <p><strong>Host Star Mass:</strong><br> ${
                  planet.host_star_mass || 'Unknown'
                }</p>
                <p><strong>Host Star Temperature:</strong><br> ${
                  planet.host_star_temperature || 'Unknown'
                } K</p>
            </div>
        </div>
