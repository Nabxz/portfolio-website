...
document.getElementById('trending').addEventListener('click', function () {
  ...
  let trendingArticles = articlesArray.filter(article =>
...
document.getElementById('my-list').addEventListener('click', function () {
  ...
  let mySavedArticles = articlesArray.filter(article =>
...
function setActiveFilter (selectedElement) {
  ...
  document
    .querySelectorAll('.article-page-filter-container-filters div')
    .forEach(filter => {
...
document.getElementById('sort').addEventListener('change', function () {
  ...
  if (currentFilter === 'all') {
    ...
    articlesToSort = articlesArray.filter(article =>
      trendingIds.includes(article.id)
...
function populateArticles (sortedArray) {
  ...
  sortedArray.forEach(article => {
    ...
    const saveButtonImage = isSaved
      ? '../assets/images/Articles/save-icon-active.webp'
      : '../assets/images/Articles/save-icon.webp'
  ...
  fetch('http://127.0.0.1:3000/wishlist/add', {
    ...
    .then(response => {
      if (!response.ok) {
        if (response.status === 403) {
          ...
          showPopupMessage(
            'Please sign in to save articles to your list.',
            true
          )
        } else {
    ...
    .then(data => {
      if (data && data.success) {
        ...
        buttonElement.querySelector('img').src =
          '../assets/images/Articles/save-icon-active.webp'
